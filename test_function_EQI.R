...
# select eploration/exploitation ratio (see EQI paper for details)
beta <- .8
...
for(i in Nsteps:1){
plot(y_plot[(i+1):nrow(y_plot),3],y_plot[(i+1):nrow(y_plot),4], xlab=expression(x[1]), ylab=expression(x[2]),
     xlim=(x_c_1_range+c(-.1,.1)), ylim=(x_c_2_range+c(-.1,.1)), main=paste("Iteration", nrow(y_plot)-nrow(orig_design_X)-i+1) , pch=19, cex=3, col=observ)
